...
export default function RafflePage() {
    ...
    const { postData, raffles, newprices, loading, isSuccess, currentPage, recordsTotal } =
    ...
    const handleChangePage = (event, newPages) => {
        ...
        // let newPage = postData.currentPage + 1;
        let newPostdata = { ...postData, currentPage: newPages + 1 }

        // console.log("newPage>>>jay", newPages);
        // console.log("newPostdata>>>jay", newPostdata.currentPage);

        // let abc = newPostdata.currentPage;

        //setPage(currentPage);

        let newPostdataa = { skip: 0, limit: 5, currentPage: newPages }

        setPage(newPages);

        //dispatch(setParams(newPostdataa));
        dispatch(getRafflesAdmin(newPostdataa))
        // return;


        // if (recordsTotal == recordsTotal) {
        //     //console.log("AAAAAAAAA");
        //     //let newPostdata = { hasMore: false }
        //     dispatch(setParams(newPostdataa));
        //     dispatch(getRafflesAdmin(newPostdataa))
        //     return;
        // } else {
    ...
    const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - recordsTotal) : 0;
    const filteredUsers = applySortFilter(raffles, getComparator(order, orderBy), filterName);
