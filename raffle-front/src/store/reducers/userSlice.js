...
export const addUser = createAsyncThunk(
    ...
    async (UserInfo, thunkAPI) => {
        let userdata = await axios.post(
            `${CONFIG.WEB_API}checkExistEmail`,
            UserInfo
        );
...
export const loginUser = createAsyncThunk(
    ...
    async (loginInfo, thunkAPI) => {
        ...
        if (userid) {
            ...
            let user = await axios.post(`${CONFIG.WEB_API}userbyId`, {
                user_id: user_id,
            });
            return user.data.user;
        } else {
            let userdata = await axios.post(
                `${CONFIG.WEB_API}loginUser`,
                loginInfo
            );
...
export const twittelLogin = createAsyncThunk(
    ...
    async (user_id, thunkAPI) => {
        let userdetails = await axios.get(
            `${CONFIG.AUTH_PATH}login/success?userid=${user_id}`,
            {
                headers: {
...
export const userSlice = createSlice({
    ...
    extraReducers: {
        ...
        [updateRaffleInfo.fulfilled]: (state, action) => {
            ...
            let exIndex = exist.findIndex(
                (e) => e.raffleId == action.payload.raffleId
            );
            if (exIndex < 0) {
